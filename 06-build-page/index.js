...
/*Вконце роботы скрипта выскакивает ощибка как её отловить так и не разобрался
 ...
 /*Нашел как с помощью beforeExit удалять содержимое папки project-dist до перезаписи*/
...
process.on('beforeExit', () => {
    ...
    console.log(
        'Формирование содержимого папки project-dist'
    );
});
                            ...
                            if (c === files4.length) {
                                ...
                                writeFile(String(__dirname + '/project-dist/index.html'), String(b), (err) => {
                                    ...
                                    readFile(data3, async (err, data3data) => {
                                        ...
                                        readFile(data3, async (err) => {
                                            if (err) throw err;                                            
                                            exit(console.log('Запись файлов завершена'))
                                        })
                                    })
                                });
                            }
